...
suspend fun main() {
    ...
    withWiremock { wiremock, port ->
        ...
        sqsAsyncClient
            ...
            // If the queue returns an empty response, the consumer resets parallelism until data is returned again
            ...
            .alsoTo {
                ...
                map { post("${numbersApiUrl(port)}/api/numbers") { stringBody(it.body()) } }
                    // Send the request to the API with a maximum parallelism of 25
                    .sendAndHandle(bodyHandler = ofString, parallelism = 25)
            ...
            // It's possible to customize the message deletion parallelism as well
